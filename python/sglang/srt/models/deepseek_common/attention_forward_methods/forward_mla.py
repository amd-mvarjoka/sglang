...
class DeepseekMLAForwardMixin:
    ...
    def forward_absorb_core(
        ...
        if self.current_attention_backend in FORWARD_ABSORB_CORE_ATTENTION_BACKENDS:
            if (
                self._skip_rope_for_nsa_tilelang_fused()
                or self._skip_rope_for_nsa_aiter_fused()
                ...
                # For tilelang/aiter NSA on ROCm, this eliminates the
                # `CatArrayBatchedCopy<OpaqueType<1u>, ...>` kernel that used to
                # fire once per layer per decode step (~2.6 us / layer saved).
                # When kv_cache_dtype is fp8, q_cat is already in fp8 (from
                # fused_qk_rope_cat_and_cache_mla with q_out_dtype=fp8_dtype),
                # so _forward_aiter skips its scaled_fp8_quant conversion.
    ...
    def _skip_rope_for_nsa_aiter_fused(self: DeepseekV2AttentionMLA) -> bool:
        """
        Check if we should skip rope and use fused rope+cache path for aiter NSA on gfx95.
        ...
        return (
            _use_aiter_gfx95
            and self.current_attention_backend == "nsa"
            and server_args.nsa_decode_backend == "aiter"
        )
